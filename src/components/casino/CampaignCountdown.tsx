...
const CampaignCountdown = ({ onExpired }: CampaignCountdownProps) => {
  ...
  return (
    <div className="relative overflow-hidden">
      ...
      <div className="relative flex items-center justify-center gap-3 px-3 py-2">
        {/* Mini progress ring */}
        <div className="relative w-[28px] h-[28px] flex-shrink-0">
          <svg className="w-full h-full -rotate-90" viewBox="0 0 44 44">
            <circle cx="22" cy="22" r={ringR} fill="none" stroke="hsl(var(--secondary))" strokeWidth="3.5" opacity="0.2" />
            <circle cx="22" cy="22" r={ringR} fill="none" stroke="hsl(var(--primary))" strokeWidth="3.5" strokeLinecap="round" strokeDasharray={ringC} strokeDashoffset={ringOffset} className="transition-all duration-1000 ease-linear" style={{ filter: "drop-shadow(0 0 4px hsl(38 95% 58% / 0.4))" }} />
          </svg>
        ...
        <p className="text-[9px] uppercase tracking-[0.15em] text-primary font-semibold flex items-center gap-1">
        ...
        <div className="flex items-center gap-0.5">
          {units.map((u, i) => (
            <div key={u.label} className="flex items-center">
              <div className="w-[28px] h-[34px] rounded bg-secondary/30 border border-border/30 flex flex-col items-center justify-center">
                <span className="text-sm font-extrabold text-foreground font-mono leading-none tabular-nums">{u.value}</span>
                <span className="text-[5px] text-muted-foreground/50 uppercase tracking-wider mt-0.5">{u.label}</span>
              </div>
              {i < units.length - 1 && (
                <span className="text-primary/50 font-bold text-[10px] w-[10px] text-center animate-pulse">:</span>
              )}
            </div>
          ))}
        </div>
