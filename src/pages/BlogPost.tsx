  ...
  return content
    ...
    .map((block) => {
      ...
      if (trimmed.startsWith("### "))
        return `<h3 class="text-xl font-bold text-foreground mt-8 mb-3">${trimmed.slice(4)}</h3>`;
      if (trimmed.startsWith("## "))
        return `<h2 class="text-2xl font-bold text-primary mt-10 mb-4">${trimmed.slice(3)}</h2>`;
      if (trimmed.startsWith("- ") || trimmed.startsWith("1. ")) {
        ...
        return `<${tag} class="${cls} pl-6 my-4 space-y-2">${items.join("")}</${tag}>`;
      }
      return `<p class="text-muted-foreground leading-relaxed my-4">${boldify(trimmed)}</p>`;
    })
    .join("");
};
